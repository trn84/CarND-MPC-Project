...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Cost from actions, factor 100 to increase impact
    for (int i = 0; i < N-1; i++) {
      fg[0] += 100*CppAD::pow(vars[delta_start + i], 2);
      fg[0] += 100*CppAD::pow(vars[a_start + i], 2);
    }

    // Cost for sequential actions, factor 1000 to increase impact
    for (int i = 0; i < N-2; i++) {
      fg[0] += 1000*CppAD::pow(vars[delta_start + i] - vars[delta_start+i+1], 2);
      fg[0] += 1000*CppAD::pow(vars[a_start + i] - vars[a_start+i+1], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set all upper and lower limits
